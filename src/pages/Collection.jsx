...
import { Search, SlidersHorizontal, ArrowUpDown, Check, X, ChevronDown } from "lucide-react";
...
const Collection = () => {
    ...
    const FilterContents = () => (
        <>
            ...
            {/* FIX: larger tap targets on mobile (p-3 vs p-2), easier to check boxes */}
            <div className="space-y-1.5 max-h-[60vh] lg:max-h-[260px] overflow-y-auto pr-1 subtle-scrollbar">
                {filteredSubcategoriesList.map((subCat) => {
                    ...
                    return (
                        <label
                            key={subCat}
                            className={`flex items-center justify-between gap-3 cursor-pointer p-3 lg:p-2 rounded-xl transition-all group border ${
                                isChecked 
                        ...
                        >
                            <div className="flex items-center gap-3 min-w-0">
                                <input
                                    ...
                                    className="hidden"
                                />
                                {/* FIX: slightly larger checkbox on mobile for easier tapping */}
                                <div
                                ...
                                <span className={`text-sm lg:text-[13.5px] truncate transition-colors ${isChecked ? 'font-bold' : 'font-medium group-hover:text-gray-900'}`}>
                                    {subCat}
                                </span>
                            </div>
                        </label>
    ...
    // Sort options config — single source of truth
    const sortOptions = [
        { value: "relevant",  label: "Relevance",    shortLabel: "Relevance" },
        { value: "low-high",  label: "Price: Low → High", shortLabel: "Low → High" },
        { value: "high-low",  label: "Price: High → Low", shortLabel: "High → Low" },
        { value: "newest",    label: "Newest",        shortLabel: "Newest" },
    ];
    const activeSortLabel = sortOptions.find(o => o.value === sortOption)?.shortLabel || "Sort";

    return (
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-5 lg:py-10 font-sans selection:bg-amber-100 selection:text-[#A47A46]">
            ...
            <div className="text-center mb-5 lg:mb-10">
                ...
                <div className="w-12 h-1 bg-[#A47A46] mx-auto mt-2.5 rounded-full" />
                <p className="hidden sm:block text-gray-500 text-xs sm:text-sm mt-2.5 max-w-md mx-auto px-2">
                ...
                {(categoryFromUrl || subcategoryFromUrl || textSearchFromUrl) && (
                    <span className="flex flex-wrap justify-center gap-1.5 mt-2.5 px-2">
                        {categoryFromUrl && (
            ...
            <div className="grid grid-cols-12 gap-0 lg:gap-8 items-start">
                ...
                {/* ── 2. MOBILE STICKY TOOLBAR ──
                    FIX: `top-[calc(var(--navbar-h,56px)+var(--trustbar-h,36px))]` is fragile
                    so we use top-[92px] which accounts for TrustBar (~36px) + Navbar (~56px).
                    Adjust this single value if your navbar height differs.
                ── */}
                <div className="col-span-12 lg:hidden mb-3 sticky top-[92px] z-30 bg-white/95 backdrop-blur-md -mx-3 px-3 border-b border-gray-100 shadow-sm">
                    <div className="flex items-center gap-2 py-2">
                        ...
                        {/* SORT pills — horizontal scroll so all options are visible */}
                ...
                <div className="col-span-12 lg:col-span-9">
                    ...
                    {/* Desktop sort/count bar — redesigned with pill buttons */}
                    ...
                    {processedProducts.length === 0 && (
                        <div className="text-center py-12 lg:py-24 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 mt-2">
                            ...
                            <button onClick={clearAllFilters} className="mt-4 px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-slate-800 transition-colors">
                                Reset All Filters
                            </button>
                        </div>
                    )}
                    ...
                    <div className="relative bg-white w-full rounded-t-[2rem] max-h-[90vh] flex flex-col shadow-2xl z-10 overflow-hidden">
                        ...
                        <div className="px-5 pb-3 border-b border-gray-100 flex items-center justify-between shrink-0">
                            ...
                            <div className="flex items-center gap-4">
                                {selectedSubcategories.length > 0 && (
                                    <button onClick={clearAllFilters} className="text-xs font-bold text-rose-500 underline underline-offset-2">
                                        Clear All
                                    </button>
                                )}
                                <button onClick={() => setIsMobileFilterOpen(false)} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 active:bg-gray-200 transition-colors">
                                    <X size={16} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-50 border-t border-gray-100 shrink-0">
                            <button onClick={() => setIsMobileFilterOpen(false)} className="w-full bg-[#A47A46] active:bg-[#8A6336] text-white text-center font-bold text-sm py-4 rounded-xl shadow-sm tracking-wide transition-colors">
